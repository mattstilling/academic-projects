...
/**
 * This class implements a weighted graph that stores vertices and edges (with numerical weights)
 * between those vertices. It uses Dijkstra's algorithm to determine the shortest path between two
 * vertices within the graph.
 * 
 * @author Matt Stilling
 * @param <T> the type of data stored within the vertices of the graph
 */
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.dataSequence = new LinkedList<>();
            for (T vertex : copyPath.dataSequence) { // Copy existing path sequence
                this.dataSequence.add(vertex);
            }
            this.dataSequence.add(extendBy.target.data); // And add the vertex this path extends to
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if (!this.containsVertex(start)) {
            throw new NoSuchElementException("The starting vertex is not in the graph");
        }
        if (!this.containsVertex(end)) {
            throw new NoSuchElementException("The ending vertex is not in the graph");
        }
        ...
        while (!pq.isEmpty()) {
            ...
            if (current.end.data.equals(end)) return current; // Smallest possible path
            if (visited.contains(current.end)) continue; // Shortest path already found for this vertex
            visited.add(current.end);
            
            for (Edge successor : current.end.edgesLeaving) {
                if (!visited.contains(successor.target)) {
                    pq.add(new Path(current, successor));
                }
            }
        }
