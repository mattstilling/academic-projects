 ...
 * This class implements the insert functionality of a RBT, but is not (yet) capable of deleting 
 * nodes.
 * 
 * @author Matt Stilling
 * @param <T> the data type to be stored in the RBT
 */
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
     ...
     * 
     * @author Matt Stilling
     * @param <T> the data type to be stored in the node
     */
    protected static class Node<T> {
        ...
        /**
         * Constructor that creates a new RBT node holding the given data. Starts as a red node.
         * 
        ...
        /**
         * @return true when this node has a parent and is the left child of that parent, 
         ...
         * order traversal. The toString of the RedBlackTree class below produces an in-order
    ...
    /**
     * Inserts the given data into the RBT, and then self-balances the tree. This tree will not hold
     * null references, nor duplicate data values.
     * 
    ...
    public boolean insert(T data) throws NullPointerException, IllegalArgumentException {
        ...
        if (root == null) { // add first node to an empty tree
            root = newNode;
            size++;
            root.isBlack = true; // set root to black before returning
            return true;
        } else {
    ...
    /**
     * Recursive helper method that resolves any red property violations and then fixes any other 
     * RBT property violations that may occur as a result.
     * 
     * @param newNode the red node that we are fixing properties for
     */
    private void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
        if (root.equals(newNode) || newNode.parent.isBlack) // P is black (or doesn't exist)
            return; // No change necessary
            // Note: also takes care of case where parent is root (since root is black)
        
        // P is red => red property violation
        if (newNode.parent.parent.leftChild != null 
                && newNode.parent.parent.leftChild.equals(newNode.parent)) {
            // In left subtree of grandparent 
            if (newNode.parent.parent.rightChild == null
                    || newNode.parent.parent.rightChild.isBlack) { // P's sibling is black or null
                
                if (newNode.parent.leftChild != null && newNode.parent.leftChild.equals(newNode)) {
                    // Case 1: need to do R rotation (sibling opposite side)
                    rotate(newNode.parent, newNode.parent.parent);
                    // Color swap nodes from rotation
                    newNode.parent.isBlack = true;
                    newNode.parent.rightChild.isBlack = false;
                    
                } else { // Case 2: need to do L-R rotation (sibling same side)
                    rotate(newNode, newNode.parent); // Left rotation
                    rotate(newNode, newNode.parent); // Right rotation
                    // Color swap nodes from right rotation
                    newNode.isBlack = true;
                    newNode.rightChild.isBlack = false;
                }
                
            } else { // Case 3: P's sibling is red
                // Re-color P and its sibling to black
                newNode.parent.isBlack = true;
                newNode.parent.parent.rightChild.isBlack = true;
                // Re-color grandparent to red, recursively fix any red property violations
                newNode.parent.parent.isBlack = false;
                enforceRBTreePropertiesAfterInsert(newNode.parent.parent);
            }
            
        } else { // In right subtree of grandparent (mirror cases of above)
            if (newNode.parent.parent.leftChild == null
                    || newNode.parent.parent.leftChild.isBlack) { // P's sibling is black or null
                
                if (newNode.parent.rightChild != null 
                        && newNode.parent.rightChild.equals(newNode)) {
                    // Case 1: need to do L rotation (sibling opposite side)
                    rotate(newNode.parent, newNode.parent.parent);
                    // Color swap nodes from rotation
                    newNode.parent.isBlack = true;
                    newNode.parent.leftChild.isBlack = false;
                    
                } else { // Case 2: need to do R-L rotation (sibling same side)
                    rotate(newNode, newNode.parent); // Right rotation
                    rotate(newNode, newNode.parent); // Left rotation
                    // Color swap nodes from left rotation
                    newNode.isBlack = true;
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        if (parent.leftChild == child) {
            ...
            if (parent.parent != null) { // parent is not root node
                ...
                root = child; // so child is new root
            }
            ...
            throw new IllegalArgumentException(
                    "Provided child must be right or left child of the provided parent");
        }
    }
